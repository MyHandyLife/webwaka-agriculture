"""
WebWaka Agriculture Sector - API Routes
RESTful API endpoints with African optimization and cultural integration
"""

from flask import Blueprint, jsonify, request
from models.agriculture_models import db, AgricultureEntity
...
def get_all_entities():
    """Get all agriculture entities with African optimization"""
    try:
        entities = AgricultureEntity.query.all()
        return jsonify({
            'success': True,
...
def get_entity(entity_id):
    """Get specific agriculture entity"""
    try:
        entity = AgricultureEntity.query.get_or_404(entity_id)
        return jsonify({
            'success': True,
            'data': entity.to_dict(),
            'african_optimized': True
...
def create_entity():
    """Create new agriculture entity with African optimization"""
    try:
        data = request.get_json()
        
        entity = AgricultureEntity(
            name=data.get('name'),
        ...
        return jsonify({
            'success': True,
            'data': entity.to_dict(),
            'message': 'Agriculture entity created successfully',
            'african_optimized': True
        }), 201
...
def update_entity(entity_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'data': entity.to_dict(),
            'message': 'Agriculture entity updated successfully'
        })
...
def delete_entity(entity_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Agriculture entity deleted successfully'
        })
...
def get_mobile_optimized():
    ...
    try:
        entities = AgricultureEntity.query.filter_by(mobile_optimized=True).all()
        return jsonify({
            'success': True,
            'data': [entity.to_dict() for entity in entities],
...
def get_traditional_approved():
    ...
    try:
        entities = AgricultureEntity.query.filter_by(traditional_authority_endorsement=True).all()
        return jsonify({
            'success': True,
            'data': [entity.to_dict() for entity in entities],
...
def sector_health():
    """Health check for agriculture sector"""
    return jsonify({
        'sector': 'agriculture',
        'status': 'healthy',
        'database': 'connected',
